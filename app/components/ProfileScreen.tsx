        ...
        >
            <div className="max-w-[800px] w-full flex-shrink-0">
                <div className="flex flex-col items-center justify-center text-center mb-20">
                    <div className="bg-gold text-bg font-bebas text-sm tracking-[0.4em] px-4 py-1 mb-6">CONFIDENTIAL RECORD</div>
                    <h2 className="font-bebas text-5xl md:text-7xl text-sand2 tracking-widest mb-6">TRAVELER PASSPORT</h2>
                    <div className="font-share text-muted tracking-[0.2em] uppercase text-xs md:text-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
                        {collection.map((item, i) => {
                            ...
                            return (
                                <motion.div
                                    key={item.id}
                                    className={`bg-panel border ${isTapped ? 'border-gold' : 'border-border md:hover:border-gold'} p-8 relative group overflow-hidden cursor-pointer transition-colors duration-300`}
                                    whileHover={{ y: -5 }}
                                    onClick={() => setTappedIndex(isTapped ? null : i)}
                                >
                                    {/* Background glow on hover or tap */}
                                    <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(212,160,23,0.1)_0%,transparent_70%)] transition-opacity duration-300 ${isTapped ? 'opacity-100' : 'opacity-0 md:group-hover:opacity-100'}`} />

                                    <div className="flex justify-between items-start mb-6 relative z-10">
                                        <div className="font-share text-xs text-muted tracking-[0.3em] uppercase">{item.city}</div>
                                    ...
                                    <div className={`absolute -bottom-1 -right-1 w-6 h-6 border-b border-r border-gold transition-opacity duration-300 ${isTapped ? 'opacity-100' : 'opacity-0 md:group-hover:opacity-100'}`} />
                                    <div className={`absolute -top-1 -left-1 w-6 h-6 border-t border-l border-gold transition-opacity duration-300 ${isTapped ? 'opacity-100' : 'opacity-0 md:group-hover:opacity-100'}`} />
                                </motion.div>
                            )
                        })}
                    </div>
